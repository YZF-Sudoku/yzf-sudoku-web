...
- In ordinary Value/Candidate solving, **fixed givens are read-only lookup targets**: tapping a given never opens the focus-follow keypad; it selects the cell and drives same-digit highlighting from that clue. User-entered values remain editable and still open the keypad, while moving focus to an empty cell clears stale digit highlighting.
- **More / 更多 → Focus-follow keypad / 焦点跟随操作盘** moves the live 1–9 and action controls into a 4×4 pad near the selected cell. The last row is **Clear / 清除 | Hint→Apply / 提示→应用 | Marks / 标记 | More / 更多**. Hint and Apply share one stateful button: it previews first, changes to Apply only while a valid hint is pending, then returns to Hint after the step is applied or invalidated. The pad uses roughly one cell per key. Its 81 cell-anchor positions are precomputed once per layout: top rows prefer below, bottom rows prefer above, side regions prefer inward, and edge overflow slides along the boundary instead of jumping to a distant board corner. Ordinary taps only look up the cached position; resize/orientation/fullscreen or board-size changes rebuild the map. Turning the option off restores the fixed controls.
...
- **Touch/mobile:** uses a cell-first workflow: tap the target cell, then press a digit in the current Value/Candidate mode. Fixed givens are lookup-only and feed same-digit highlighting without opening the editing pad; editable cells keep the normal controls. Focus-follow is only a control-placement option; it does not change this ordering.
...
YZF Sudoku 的原创源代码、用户界面、文档、中文翻译、测试资料及新增功能受著作权法保护。未经权利人书面许可，不得擅自复制、修改、重新发布、镜像托管、商业利用或删除版权及署名信息。

项目包含的第三方程序、开源组件、既有算法实现及相关资料，其权利归相应权利人所有，并适用各自的许可条款。本声明不扩大或改变第三方组件原有许可证授予的权利。

联系邮箱：63160007@qq.com

© 2019–2026 YZF Sudoku and its developers. All rights reserved.

The original source code, user interface, documentation, Chinese translations, test materials, and newly developed features of YZF Sudoku are protected by copyright law. Without prior written permission from the rights holder, they may not be copied, modified, republished, mirror-hosted, commercially exploited, or stripped of copyright or attribution information.

Third-party programs, open-source components, pre-existing algorithm implementations, and related materials included in this project remain the property of their respective rights holders and are subject to their own license terms. This notice does not expand or alter the rights granted by those third-party licenses.

Contact: 63160007@qq.com

The full notice is also published in [`COPYRIGHT.md`](COPYRIGHT.md).
