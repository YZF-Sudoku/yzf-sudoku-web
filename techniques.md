...
- 唯一解：若某结构成立会产生两个可互换完成盘，则唯一解题必须排除它。
...
- 鱼与复杂鱼： X-Wing、Swordfish、Jellyfish、Finned/Complex/Multi-Fish。始终只盯一个数字，检查 base 与 cover 的覆盖关系。

- 链理论： X-Chain、XY-Chain、AIC、ALS Chain、Force Chain、Whip/Braid。沿强弱关系交替读，重点看两端或矛盾点。

- 构造视角： Wing、Broken Wing、Blossom、Kraken/Burred/Blossom Loop。把它们看成“核心结构 + 附加支撑”，不要把所有高亮都当成同一级。

- 秩、飞鱼、致命结构： MSLS、Oddagon、Exocet、UR/UL/AR/SK Loop。重点看例外点、守护者、唯一解前提，以及结论是否只落在共同受限位置。
...
- **基本含义：**GSP 用唯一解或全局摆放对称性，排除某些看似可行但会导致非唯一或等价变化的候选。

- **成立逻辑：**用户可以把它理解成一种“全局唯一性检查”：若某组候选按某种方式保留，会制造可替换的第二种完成方式。

- **数学逻辑：**这是基于唯一解的全局可替换性推理。若高亮结构允许一组候选互换而不影响其他格，则同一题会产生两个完成盘；唯一解题不允许这种对称自由度，因此破坏唯一性的候选组合必须被排除。

- **看盘步骤：**① 找高亮的成组位置；② 看它们是否形成可替换关系；③ 确认唯一解前提；④ 应用系统给出的删数。

- **高亮阅读：**高亮通常会标出一组互相呼应的位置；重点不是单个格子，而是整组位置的同步关系。

- **核对要点：**只在确认题目是唯一解时使用；若是多解/无解盘面，GSP 类结论不应作为手工逻辑依据。
...
- **成立逻辑：**若多出的候选也被去掉，盘面会接近双值循环并产生非唯一风险；因此多出的候选通常被确认。

- **数学逻辑：**BUG 的数学核心是双值图的奇偶平衡：若全盘所有未定格都是双值，且每个候选在每个区域中也恰好出现两次，就会形成一个可二染色的双解结构。BUG+1 只有一个额外候选能破坏这种双解；为了保持唯一解，该额外候选必须成立，或等价地删除同格其他候选。

- **看盘步骤：**① 看全盘双值状态；② 找唯一异常格；③ 找异常候选；④ 按结论出数或删数。

- **高亮阅读：**高亮重点是唯一的异常格和异常候选。

- **核对要点：**它依赖唯一解前提；先确认除异常格外其他空格是否基本双值。
...
- **数学逻辑：**BUG+n 是 BUG+1 的推广。若去掉若干额外候选后会得到一个全双值、可互换的 BUG 双解结构，那么这些额外候选不可能全部为假；系统给出的删数来自“保持至少一个破坏点成立”之后对同格或同区域的排除。
...
**Kind:** Fireworks **Category:** Single Digit **Difficulty:** 5 已接入 / wired

- **基本含义：**烟花把一个宫角或交叉区域内的多个候选看成会向行列“发射”的集合。

- **成立逻辑：**若某些候选不能留在核心位置，就会被迫出现在对应行/列；这种同步限制可以产生删数。

- **数学逻辑：**Fireworks 的核心是“一个小结构内部必有某种逃逸/落点”。当若干候选在行列宫交叉处形成烟花结构时，数字不能同时避开所有高亮出口；因此某些目标格若保留指定候选，会堵死所有合法分配，或与必然出口冲突。
...
对应 Wing 与构造章节。Wing 类通常是小核心加两个或多个端点；删数必须被这些端点共同看见或共同限制。
...
- **基本含义：**两个含有同一候选的双值格，通过该候选在某区域中的强关系连接。

- **成立逻辑：**两个端点不可能同时避开目标数字；因此能同时看见两个端点的该数字候选可删。

- **数学逻辑：**W-Wing 是两个同候选双值格加一条同数字强链。若两个翅膀中共同数字不在一端成立，强链会迫使另一端成立；无论哪种情况，两个端点中的至少一个会取共同候选，所以同时看见两端的该候选可删。
...
- **基本含义：**四候选 Wing，是 XY/XYZ-Wing 的更大规格推广。

- **成立逻辑：**多个翼格与核心格共同限制一组候选；某个目标候选若成立，会让核心结构无合法分配。

- **数学逻辑：**WXYZ-Wing 是四候选集合的分支压缩。枢纽/翅膀组合保证无论核心格最终取哪一个候选，某个外侧关键候选都会成立；因此能同时看见所有相关落点的同候选目标没有合法分支可存活。

- **看盘步骤：**① 看核心集合；② 确认候选种类；③ 找共同目标；④ 删除被所有端点限制的候选。

- **高亮阅读：**高亮可能包含 4 个以上格；先看核心候选集合，再看目标候选。

- **核对要点：**检查目标是否被所有必要端点共同限制。


### Broken Wing 破损翅膀

**Kind:** BrokenWing **Category:** Wings **Difficulty:** 5 已接入 / wired

- **基本含义：**Broken Wing 是标准 Wing 被额外条件“修补”或“破损后仍可用”的构造。

- **成立逻辑：**它通常把一个不完整 Wing 加上 guardian、额外强关系或共同可见条件，使结论重新成立。

- **数学逻辑：**Broken Wing 可以看成“差一点成立的 Wing + guardian/修补点”。若 guardian 不成立，标准 wing 推理产生删数；若 guardian 成立，目标候选又因可见或区域限制被排除。删数必须同时被这两个分支覆盖。

- **看盘步骤：**① 找 Wing 骨架；② 找破损/例外点；③ 看补充支撑；④ 只删共同保证的目标。

- **高亮阅读：**高亮会同时出现 Wing 核心和补充支撑；蓝色/辅助结构不要当成普通删数本体。

- **核对要点：**重点核对额外支撑是否真的覆盖了破损处；否则不能按普通 Wing 删除。


## 唯一性技巧 / Uniqueness

对应唯一性与致命结构理论。它们使用“合格题唯一解”作为前提，目标是阻止一个可互换、可反转或多解的危险形态成立。
...
- **成立逻辑：**如果某候选成立会导致两个解可互换，唯一解题中它就不能成立。

- **数学逻辑：**这是唯一解反证。若删数候选被保留或某些额外候选全被去掉，结构会退化成只含两个数字的可交换闭环；这会产生两个只在该结构内互换、其他格完全相同的完成盘。唯一解题不允许这种“致命形态”，所以必须保留破坏点，或删除会导致致命形态的候选。

- **看盘步骤：**① 找矩形四角；② 看候选对；③ 判断危险互换；④ 删除造成危险的候选。

- **高亮阅读：**高亮矩形四角和导致危险形态的候选。

- **核对要点：**只适用于唯一解题；确认矩形不是跨四宫导致推理失效的形态。
...
- **基本含义：**唯一矩形使用四个分布在两行两列两个宫内的格子，避免两个数字形成可互换的第二解。

- **成立逻辑：**若额外候选被去掉后四格只剩同一对数字，就会出现两种互换完成方式；唯一解题不允许这种状态。
...
- **成立逻辑：**若环上只剩两个数字交替排列，会产生另一种等价解；额外候选必须破坏该环。
...
- **核对要点：**不要只凭“像矩形”判断；必须能解释为什么会产生第二解。


## Oddagon / 奇环结构

对应 guardian / oddagon 思路。读法是先看奇环为什么危险，再看 guardian 或额外候选如何破坏危险结构。


### Bivalue Oddagon 双值奇环

**Kind:** BivalueOddagon **Category:** Oddagon **Difficulty:** 5 已接入 / wired

- **基本含义：**双值奇环由一圈双值关系构成，若没有破坏点会造成奇数环矛盾或非唯一风险。

- **成立逻辑：**奇环无法用两个值稳定交替闭合，因此必须存在某个 guardian/额外候选来打破它。

- **数学逻辑：**Bivalue Oddagon 是奇环不可二染色的矛盾。若奇数个双值格只围绕两个数字交替约束，沿环传播真假会在回到起点时要求同一候选既真又假；因此让该奇环完整成立的候选组合不可能出现，相关候选可删。

- **看盘步骤：**① 沿环读双值关系；② 确认奇环；③ 找 guardian；④ 删除被所有必要条件排除的候选。

- **高亮阅读：**高亮环本体和 guardian；删数通常来自所有 guardian 的共同影响。

- **核对要点：**确认环长度和双值关系；不要把 guardian 当成普通环节点。


### Triplet Oddagon 三数字奇环

**Kind:** TripletOddagon **Category:** Oddagon **Difficulty:** 8 已接入 / wired

- **基本含义：**三数字奇环把双值奇环推广到三个数字或三值结构。

- **成立逻辑：**若三数字结构按危险方式闭合，会迫使某些 guardian 或异常候选承担破坏作用。

- **数学逻辑：**Triplet Oddagon 是双值奇环的三数字推广。若高亮结构被限制成三数字的固定循环，沿奇数环分配会在某处造成重复或缺位；系统删数的含义是：该候选若成立，就会把结构推入这种不可完成的奇环。

- **看盘步骤：**① 看三数字集合；② 沿奇环读结构；③ 找 guardian/异常点；④ 应用共同结论。

- **高亮阅读：**高亮可能有三种候选和多个 guardian；先分清环数字集合。

- **核对要点：**核对结论是否来自全部 guardian 或必要分支，而不是只看其中一条线。
...
**Kind:** BlossomLoop **Category:** ALS **Difficulty:** 7 已接入 / wired

- **基本含义：**花环是 Blossom 结构闭合成环后的形态。

- **成立逻辑：**环上每个 ALS/分支都互相支撑，闭合后产生更强的共同删数或出数。

- **数学逻辑：**Blossom Loop 把 Death Blossom 的分支首尾闭合。环内每个 ALS 都把一个候选的真假传给下一个结构；若目标候选成立，整圈传播后会回到起点并产生真假矛盾或容量超限，因此目标候选可删。

- **看盘步骤：**① 找主环；② 顺序读 ALS/RCC；③ 分清分支；④ 应用环整体结论。

- **高亮阅读：**高亮会包含主环和可能的分支；先沿主环读，再看旁支。

- **核对要点：**确认结论来自闭环整体，而不是单个 ALS。
...
- **数学逻辑：**Force Chain 是完备分支推理。对某个格或区域，所有可能候选/位置必须有且只有一个成立；如果逐一假设每个分支后都会推出同一个删数或同一个出数，那么无论真实分支是哪一个，该共同结论都必然成立。
...
对应秩理论。用户不必计算矩阵，只要理解“强区域提供名额、弱区域消耗名额”，秩越低，外部删数越强。
...
- **基本含义：**SK Loop 是一种覆盖/环结构，常可按秩或唯一性视角理解。

- **成立逻辑：**若环状覆盖占满必要名额，环外或交叉处多余候选就无法保留。

- **数学逻辑：**SK Loop 是唯一矩形思想和循环覆盖的结合。环上若干双候选位置会形成可交换闭环；如果某个外部候选成立，会让环退化为致命循环或造成某条边无法满足，因此这些外部候选可删。

- **看盘步骤：**① 找 SK 环区域；② 看候选对；③ 分清环内外；④ 删除外部受限候选。

- **高亮阅读：**高亮通常有多个区域和候选对；先看环，再看删数。

- **核对要点：**核对删数是否处在环结构共同影响的位置。
...
- **基本含义：**MSLS 把多个区域合成一个锁定集合。

- **成立逻辑：**强区域提供候选名额，弱区域覆盖这些名额；当覆盖足够紧，多余候选可删。

- **数学逻辑：**MSLS 是多区域锁定集合，数学上接近 rank/cover 推理。若若干 base 区域需要的候选实例全部被若干 cover 区域覆盖，且 cover 容量不允许额外实例存在，那么 cover 中不属于合法分配的候选会造成重复或超额，必须删除。

- **看盘步骤：**① 看参与区域；② 分清强/弱区域；③ 数覆盖是否锁住；④ 删除多余候选。

- **高亮阅读：**高亮会标出多个 sector/区域，删数常在弱区域多余位置。

- **核对要点：**用户可按“名额是否被占满”理解，不必计算内部矩阵。
...
- **基本含义：**秩视角的 Multi-Fish 把复数鱼当作强弱区域覆盖结构。

- **成立逻辑：**当弱区域覆盖了强区域所有必要名额，任何额外落点都会超额，因此可删。

- **数学逻辑：**这是把鱼推广为集合覆盖后的秩逻辑。base 集合要求放入若干个同数字实例，cover 集合负责接住这些实例；当 cover 的容量正好锁住 base 的全部可能位置时，cover 外或 cover 内多余位置会导致某个区域重复或某个 base 无法满足。复杂鱼的“复杂”不在结论，而在 base/cover 可由行、列、宫甚至组合区域共同构成。

- **看盘步骤：**① 看强区域；② 看弱区域；③ 找例外点；④ 删除超额候选。

- **高亮阅读：**高亮分层显示 base/cover、guardian 或 fin。

- **核对要点：**重点核对 rank 结论指向哪里：删数通常不是结构本体，而是超额位置。


## Exocet 家族 / Exocet Family

对应飞鱼章节。Exocet 的读法是先确认 base、target、cross/mirror 的角色，再看目标格候选如何被 base 同步约束。
...
- **基本含义：**Junior Exocet 由 base cells、target cells 和 crossline/cross cells 构成。

- **成立逻辑：**base 中的候选会同步限制 target；目标格必须承接 base 的某些候选，因此非同步候选或交叉处候选可删。

- **数学逻辑：**Junior Exocet 的核心是 base 数字必须经由 target/cross 结构完成配对。若 base 中某个数字成立，它在目标区的落点受到行列宫共同限制；任何会让 base 数字无法在 target 中合法对应，或迫使 cross cell 与 base 重复的候选，都可删除。

- **看盘步骤：**① 找 Base；② 找 Target；③ 看 Cross/Crossline；④ 删除不符合同步关系的候选。

- **高亮阅读：**高亮通常明确标出 Base、Target、Cross。

- **核对要点：**先确认 base 与 target 的候选集合关系，再看结论是哪一类删数。
...
- **基本含义：**Senior Exocet 是更强或更复杂的 Exocet 形态。

- **成立逻辑：**它仍围绕 base 与 target 的同步关系，只是额外条件能推出更多删数。

- **数学逻辑：**Senior Exocet 是更强约束的 Exocet。base cells 给出一组必须传递到 target cells 的数字，cross/companion 区域限制这些数字的路径；系统删数来自反证：目标候选若成立，会阻断某个 base 数字的唯一传递方式。

- **看盘步骤：**① 找 Base；② 找 Target；③ 看额外支撑；④ 应用同步删数。

- **高亮阅读：**高亮会比 Junior Exocet 更复杂，但角色仍按 base/target/cross 读。

- **核对要点：**不要把所有交叉格候选都删掉；只删结论列出的受同步限制候选。
...
- **基本含义：**Weak Exocet 是条件较弱的 Exocet 形态。

- **成立逻辑：**虽然没有完整 Exocet 那么强，但 base 与 target 之间仍存在足以推出部分结论的同步限制。

- **数学逻辑：**Weak Exocet 保留 Exocet 的 base-target 必然性，但约束较弱，所以结论通常更谨慎。它仍依赖分支覆盖：base 中每个可能数字都要在目标结构中找到合法去处；若某候选让所有去处冲突，就可删。

- **看盘步骤：**① 找弱 Exocet 骨架；② 看缺失/弱化条件；③ 找仍成立的同步关系；④ 删除被保证的候选。

- **高亮阅读：**高亮会标出弱化后的 base/target/cross 关系。

- **核对要点：**重点看“弱”在哪里，以及结论是否相应缩小。
