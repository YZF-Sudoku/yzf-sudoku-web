...
> V438 proof-detailed 版：参考 Kazusa Sudoku tutorial 的术语递进和推理说明方式，进一步补强每个已实现技巧的数学逻辑、反证理由和删数/出数必然性。AHS-XY-Wing 与 AHS-W-Wing 已接入；AHS Chain 当前仍是 reference placeholder。
...
已按当前后端 wired 技巧整理： 源码的参考技巧表共有 70 项，其中仅 AHS Chain 仍是 reference placeholder；AHS-XY-Wing 与 AHS-W-Wing 已写入已实现技巧。
...
- **基本含义：**唯一环是唯一矩形的环状推广。主体由同一对致命数字沿行、列、宫约束形成连通的偶数闭环；每个参与约束恰好连接两个主体格，因此两数字可以交替互换。

- **成立逻辑：**若环上只剩两个致命数字交替排列，会产生另一种等价解；唯一解题必须保留至少一个破坏点。普通 Type 1–4 直接使用环内额外候选；External Guardians 则把破坏点放在环外，并证明外部守护候选至少一个为真。

- **数学逻辑：**主体的每格在初始候选 `base_cand` 与当前候选中都必须保留同一对致命数字；当前格可带额外候选，但不能缺少任一致命数字。选定一族行、列或宫作为 `GuardianCover` 后，若该族全部外部 Guardians 为假，每个 cover house 中两种致命数字都只剩两个主体格可放，区域完整性会迫使两格分别取这两个数字；再由连通二分主体得到仅有的两种互补着色。因此至少一个 Guardian 必真。External Test 1、2/4、3、3H 与 External XY-Wing 再分别结合单格保留、单数字共同可见、裸数组容量、隐性数组容量或两翼共同数字得到删数。

- **看盘步骤：**① 用初始候选与当前候选共同确认致命数字对；② 检查主体为偶数、连通、可二分，且每个参与行/列/宫恰有两个主体格；③ 核对 `GuardianCover` 族中，外部致命数字候选与 `Guardians` 完全一致；④ 按 External 子型核对共同可见、数组容量或双翼条件；⑤ 只应用后端给出的目标。

- **高亮阅读：**`ULBody` 标出环主体，`GuardianCover:Rows/Columns/Boxes` 标出本次“全假即可恢复致命环”的覆盖族，`Guardians` / `GuardiansA/B` 标出外部破坏点；Test 3/3H 另标裸数组/隐性数组，XY-Wing 另标 `WingA/B`。候选色与整格角色不可混用。

- **核对要点：**确认题目唯一；主体必须是偶数、连通、可二分，原始提示格不得混入；每个主体格在 `base_cand` 和当前候选中均含完整致命数字对，额外候选允许。`GuardianCover` 必须与独立推导的覆盖族完全一致。普通 UL Type 1–4 保持优先；External 仅在严格主体、B8 守护前提和对应子型证明同时成立时输出。
...
- **基本含义：**本项目的 Almost Triple 是宫线交区两侧的 ALS+AHS 容量结构，而不是通用 ALC。

- **成立逻辑：**显性待定侧决定三个活动数字中有几个必须落入宫线交区，另一侧的隐性承载格组接住其余数字。

- **数学逻辑：**Single-Intersection 使用外侧 2 格/3 数 ALS 与另一侧 2 个 AHS 承载格，迫使交区恰占 1 个活动数字；Double-Intersection 使用外侧各 1 格，并要求三格交区全部含活动数字，迫使交区恰占 2 个。于是 ALS 同侧其余活动数字和 AHS 格中的额外候选没有容量保留。

- **看盘步骤：**① 选宫与相交行/列；② 核对 Box-ALS / Line-AHS 或反向 Branch；③ 区分 Single/Double-Intersection；④ 检查 ALS、交区、AHS 的精确格数；⑤ 应用删数。

- **高亮阅读：**活动交区候选、AHS 全部候选、ALS 全部候选与删数分别着色。

- **核对要点：**Double-Intersection 必须是三格交区全部含活动数字，且两侧域外分别恰好 1 个 ALS 格和 1 个 AHS 承载格。
...
- **看盘步骤：**① 先读候选数组合@house；② 再看这些数字的全部可落格；③ 找受限公共位置/候选；④ 删除共同目标。

- **高亮阅读：**高亮重点是数字集合及其可落格，而不是显性候选集合。

- **核对要点：**确认这是“数字被困住”，不是“格子候选少”的 ALS 读法。扩展同数格位RCC必须列出Extra事件、最多三格的局部HLS，以及支撑数字在所有合法匹配中的全部位置。


### AHS-XY-Wing AHS-XY 翅膀

**Kind:** AHSXYWing **Category:** AHS **Difficulty:** 6 已接入 / wired

- **基本含义：**三组AHS通常由两条严格rank-1 RCC连接，中间AHS充当枢纽；若外翼A与C之间还存在第三条独立RCC，则升级为 `Triple-Linked Rank-0`。

- **成立逻辑：**普通分支中，两条RCC在枢纽AHS上对应互不相交的Extra事件，迫使至少一个外翼Extra事件成立。Triple-Linked分支中，第三条A-C RCC与前两条在各端点都不复用Extra事件或Hall/HLS证明资源，三角形只剩两个交替全局状态，链接容量闭合为Rank 0。

- **数学逻辑：**普通分支为 `(E_AX∨E_BX)` 与 `(E_CY∨E_BY)`。Triple-Linked再加入 `(E_AZ∨E_CZ)`；端点互斥后只剩 `E_AX/E_BY/E_CZ` 与 `E_AZ/E_BX/E_CY` 两个状态。程序对两个状态中的全部合法AHS匹配求共同结论，既可得到结构外删数，也可得到结构内Cannibalism删数。

- **看盘步骤：**① 找枢纽AHS；② 核对X、Y两条严格rank-1 RCC；③ 确认枢纽事件互斥且证明资源独立；④ 搜索并验证外翼RCC Z；⑤ 若Z有效，验证两个交替全局状态并应用Rank-0删数，否则保留普通两分支共同删数。

- **高亮阅读：**三组AHS分别着色；X、Y、Z分别输出Extra事件、Hall见证或局部HLS格组与全部有效支撑位置，第三条RCC使用独立底色。

- **核对要点：**不能仅凭外翼共享一个格就宣称第三RCC。若共享格仍可取两组AHS共有数字，则它不是Shared-Cell RCC；三条边必须各自为真实rank-1关系，端点事件与证明资源必须独立，且至少一个交替状态可行。


### AHS-W-Wing AHS-W 翅膀

**Kind:** AHSWWing **Category:** AHS **Difficulty:** 6 已接入 / wired

- **基本含义：**一个单格枢纽把自身全部候选完整分成A侧和B侧，并分别连接两组AHS；若两端AHS之间还存在第三条独立RCC，则升级为 `Triple-Linked Rank-0`。双值枢纽只是最小特例。

- **成立逻辑：**普通分支中，A组枢纽候选强制A侧Extra事件，B组候选强制B侧Extra事件。Triple-Linked再加入端点RCC Z；同一端点的枢纽事件与Z事件互斥后，只剩“枢纽取A组、B走Z”和“枢纽取B组、A走Z”两个交替状态。

- **数学逻辑：**端点条件HLS最多三格，逐数字支撑位置来自全部合法匹配。第三条端点RCC必须是真实rank-1 Extra析取，并与枢纽端点事件及证明资源独立。程序精确枚举两个Rank-0状态下的枢纽取值与两端AHS匹配，删除所有状态都不能容纳的候选。

- **看盘步骤：**① 找至少双候选的单格枢纽；② 将全部候选完整分入A/B两侧；③ 逐数字核对两个AHS端点的全部有效支撑；④ 搜索端点RCC Z；⑤ 验证端点事件独立和两个全局状态；⑥ 有Z时应用Rank-0删数，无Z时取普通两分支共同删数。

- **高亮阅读：**枢纽A/B候选组与两侧局部HLS继续使用两种配对色；有效端点RCC Z使用第三种底色，另行显示Extra事件、Hall/HLS见证和支撑位置。

- **核对要点：**不要求枢纽恰好双值。任一枢纽候选未被分组、漏掉合法支撑位置、端点RCC不是严格rank-1、同端事件或证明资源复用、或两个全局状态都不可行，均不能标为Triple-Linked Rank-0。
