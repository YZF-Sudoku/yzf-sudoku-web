...
- **基本含义：**三组AHS由两条rank-1 RCC连接，中间AHS充当枢纽。

- **成立逻辑：**两条RCC在枢纽AHS上对应互不相交的Extra事件。枢纽只有一个额外格，因此两个枢纽事件不能同时成立，迫使至少一个外翼Extra事件成立。

- **数学逻辑：**设两条边分别为 `ExtraA(SA) OR ExtraB(SX)` 与 `ExtraC(SC) OR ExtraB(SY)`。若 `SX∩SY=∅`，且两条边没有复用同一Hall/HLS证明资源，则枢纽Extra不可能同时位于SX和SY，所以 `ExtraA(SA) OR ExtraC(SC)`。分别枚举两个外翼事件下的完整AHS匹配，取共同删数。

- **看盘步骤：**① 找枢纽AHS；② 核对两条严格rank-1 RCC；③ 确认枢纽事件互斥且证明资源不复用；④ 比较两个外翼事件的共同结论。

- **高亮阅读：**三组AHS分别着色；两条RCC应同时标出Extra事件、Hall见证或局部HLS格组与全部有效支撑位置。

- **核对要点：**不能只按“有两条边”判XY-Wing；枢纽事件必须互斥，两条边都必须独立成立。
...
- **基本含义：**一个单格枢纽把自身全部候选完整分成A侧和B侧两组，并分别通过严格同数格位端点连接两组AHS；双值枢纽只是最小特例。

- **成立逻辑：**枢纽中归入A组的每个候选，都能看见A侧对应支撑数字的全部有效位置，从而强制A侧Extra事件；归入B组的候选同理。枢纽必取其中一个候选，所以A、B两侧Extra事件至少一边成立。

- **数学逻辑：**每个端点都必须是严格的条件局部HLS：格组最多三格，逐数字支撑位置来自全部合法匹配，而不是原始候选并集。枢纽候选必须被A/B两组完整覆盖，两个Extra事件分别产生删数集合，最终只保留共同结论。

- **看盘步骤：**① 找一个至少双候选的单格枢纽；② 将它的全部候选完整分入A/B两侧；③ 找两个AHS端点；④ 逐数字核对枢纽候选能看见对应支撑的全部有效位置；⑤ 枚举两个Extra事件并删除共同目标。

- **高亮阅读：**枢纽中的A组、B组候选应分别着色，同时突出两侧Extra事件、局部HLS及逐数字精确支撑位置；目标是两个事件分支共同删数。

- **核对要点：**这不是普通强链文字替换，也不要求枢纽恰好双值。若任一枢纽候选未被分组、任一端点漏掉合法支撑位置，或枢纽候选不能看见对应数字的全部位置，结构都不成立。
